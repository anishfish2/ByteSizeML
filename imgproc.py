...
def preprocess_one_image(image_path: str, device: torch.device) -> Tensor:
    ...
    array = None
    with open(image_path) as in_file:
    ...
    for frame in array:
        ...
        tensor = image_to_tensor(frame, False, False).unsqueeze(0)

        # Transfer tensor channel image format data to CUDA device
        tensors.append(tensor.to(device=device, memory_format=torch.channels_last, non_blocking=True))
